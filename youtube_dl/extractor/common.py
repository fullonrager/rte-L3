    ...
    def _parse_mpd_formats(self, mpd_doc, mpd_id=None, mpd_base_url='', formats_dict={}, mpd_url=None):
        ...
        def extract_multisegment_info(element, ms_parent_info):
            ...
            def extract_Initialization(source):
                ...
                if initialization is not None:
                    ms_info['initialization_url'] = initialization.attrib['sourceURL'].replace('drm=1', 'drm=0')
                    print(ms_info['initialization_url'])
        ...
        for period in mpd_doc.findall(_add_ns('Period')):
            ...
            for adaptation_set in period.findall(_add_ns('AdaptationSet')):
                if "maxHeight" in adaptation_set.attrib:
                    if float(adaptation_set.attrib["maxHeight"]) > maxheight:
                        maxheight = float(adaptation_set.attrib["maxHeight"])
            for adaptation_set in period.findall(_add_ns('AdaptationSet')):
                if "maxHeight" in adaptation_set.attrib and "mimeType" in adaptation_set.attrib:
                    if adaptation_set.attrib["mimeType"] == "video/mp4" and float(adaptation_set.attrib["maxHeight"]) == maxheight:
                        pass
